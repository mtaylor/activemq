...
public class JMXMasterSlaveSharedStoreTest extends TestSupport
{
   ...
   protected AtomicReference<BrokerService> slaveAtomicReference = new AtomicReference<BrokerService>();
   ...
   {
      master = createBrokerService("master", 62001, 1098);
      master.start();
   }
   ...
   protected void createSlave() throws Exception
   {
      ...
      new Thread(new Runnable() {
         public void run() {
            try
            {
               slave = createBrokerService("slave", 62002, 1099);
               slave.start();
   ...
   private BrokerService createBrokerService(String name, int port, int jmxPort) throws Exception
   {
   ...
   private String getBrokerServiceMBeanName(String brokerName)
   {
      return "org.apache.activemq:type=Broker,brokerName=" + brokerName;
   }


   // To fix this start the schedulder
   @Test
   public void testJMXMBeanIsRegisteredForSlaveWithScheduler() throws Exception
   {
      // Ensure Configuration replicates ENTMQ-841
      assertFalse(master.isSlave());
      assertTrue(master.isSchedulerSupport());
      assertTrue(slave.isSlave());
      assertTrue(slave.isSchedulerSupport());

      // Expected MBeans:
      ObjectName masterMBeanName = new ObjectName(getBrokerServiceMBeanName("master"));
      ObjectName slaveMBeanName = new ObjectName(getBrokerServiceMBeanName("slave"));

      // Connect to JMX on master and slave, checking both master and slave MBeans are registered.
      // service:jmx:rmi:///jndi/rmi://localhost:1098/jmxrmi
      Thread.sleep(100000);
      for (Integer port : Lists.newArrayList(1098, 1099))
      {
         MBeanServerConnection connection = createJMXConnector(port);
